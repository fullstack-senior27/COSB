...
const getBeauticianById = async (id) => {
  const beautician = await Beautician.findById(id)
    ...
    .populate('reviews');
  if (!beautician) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Beautician with this ID does not exist!');
  }
  return beautician;
};
...
const filterBeauticians = async ({
  ...
  if (
    ...
    filteredResults = result.filter((beautician) => {
      ...
      if (price_range) {
        priceCondition = beautician.services.some(
          (service) => service.price >= price_range.minPrice && service.price <= price_range.maxPrice
        );
